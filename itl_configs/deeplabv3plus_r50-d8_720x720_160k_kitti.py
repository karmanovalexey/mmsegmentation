_base_ = [
    '../configs/_base_/models/deeplabv3plus_r50-d8.py', '../configs/_base_/datasets/kitti.py',
...
data = dict(
    samples_per_gpu=1,
    workers_per_gpu=1,
    train=dict(pipeline=train_pipeline, data_root='/workspace/Kitti'),
    val=dict(data_root='/workspace/Kitti'),#, split='/workspace/mmsegmentation/splits/split.txt'),
    test=dict(data_root='/workspace/Kitti')#, split='/workspace/mmsegmentation/splits/split.txt')
)
